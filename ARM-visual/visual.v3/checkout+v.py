...
#                       float(line.split()[10]) )
    ...
    if line.split()[11] != 'x':
      centerv = vector( float(line.split()[11]),\
                     float(line.split()[12]),\
                     float(line.split()[13]) )

      normalv1 = 4*vector( float(line.split()[14]),\
                       float(line.split()[15]),\
                       float(line.split()[16]) )

      normalv2 = 4*vector( float(line.split()[17]),\
                       float(line.split()[18]),\
                       float(line.split()[19]) )

      ballv      = sphere(pos=centerv, radius=0.5, color=(1,0,0))
      pointerv1  = arrow(pos=centerv, axis=normalv1, color=(1,0.5,0))
      pointerv2  = arrow(pos=centerv, axis=normalv2, color=(1,0,1))
