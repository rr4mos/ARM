...
stop
  ...
  do n = 1, ncadeiaspt
   ...
   if (R(l,m,n,o)< 10.0) then
    ...
    do i=1,5
!de cada um dos atomos para o centro do anel
     ii = o

!     ssxT = sxT(n,ii) - svx(l,m)
!     ssxT = ssx   - (dint((2.d0*ssxT +3.d0)/2.d0)-1.d0)
!     ssyT = syT(n,ii) - svy(l,m)
!     ssyT = ssyT  - (dint((2.d0*ssyT +3.d0)/2.d0)-1.d0)
!     sszT = szT(n,ii) - svz(l,m)
!     sszT = sszT  - (dint((2.d0*sszT +3.d0)/2.d0)-1.d0)

 !    xxT  = A(1,1)*ssxT +A(1,2)*ssyT +A(1,3)*sszT 
 !    yyT  = A(2,1)*ssxT +A(2,2)*ssyT +A(2,3)*sszT 
 !    zzT  = A(3,1)*ssxT +A(3,2)*ssyT +A(3,3)*sszT 
   end do
   endif
   end do
  end do 
