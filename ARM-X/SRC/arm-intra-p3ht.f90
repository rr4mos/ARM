...
TYPE (vector), dimension (:,:)    ,allocatable:: v_n, v_s, v_c, vnn
...
allocate(v_n(ncadeias,naneisolig))
allocate(v_c(ncadeias,naneisolig))
allocate(v_s(ncadeias,naneisolig))
...
do l = 1, ncadeias
   ...
   do m = 1, naneisolig
      ...
      xmv4=cx(l,atmS)-xm(l,m)        !\vec centro para S
      ymv4=cy(l,atmS)-ym(l,m) 
      zmv4=cz(l,atmS)-zm(l,m)
      v1 = (/ xmv4, ymv4, zmv4 /)
   
      xmv6=cx(l,atm5)-xm(l,m)        !\vec centro para Clink
      ymv6=cy(l,atm5)-ym(l,m) 
      zmv6=cz(l,atm5)-zm(l,m)
      v2 = (/ xmv6, ymv6, zmv6 /)

      v_s(l,m) = v1
      v_s(l,m) = v_s(l,m)/sqrt( v_s(l,m).DOT.v_s(l,m) )

      v_c(l,m) = v2
      v_c(l,m) = v_c(l,m)/sqrt( v_c(l,m).DOT.v_c(l,m) )

      v_n(l,m) = ( v_c(l,m)*v_s(l,m) ) ! \vec normal (completa a base - nao ortonormal	)
      v_n(l,m) = v_n(l,m)/sqrt( v_n(l,m) .DOT. v_n(l,m) )
	
      write(18,'(2i3,13f12.5)')l,m, xm(l,m), ym(l,m), zm(l,m),v_c(l,m),v_s(l,m),v_n(l,m),v_c(l,m).DOT.v_s(l,m)
...
open(19,file='angs.intra.1nb.dat')
open(20,file='polar.dat')
do l=1, ncadeias
   write(19,'(2i3, 3f10.5)')  l,1, 0.0, 0.0, 0.0
      write(20,'(2i3, 2f10.5)')  l,1, 0.0, 0.0
...
!Torsion (ns)
      pjn = v_n(l,m).DOT.v_n(l,m+1)
      pjs = v_s(l,m).DOT.v_n(l,m+1)
      dotp = pjn/sqrt( pjn**2 + pjs**2 )
      angT= dacos( dotp*0.9999999d0 )*(180.d0/PI)

!Out of plane (nc) 
      pjn = v_c(l,m).DOT.v_n(l,m+1)
      pjc = v_n(l,m).DOT.v_n(l,m+1)
      dotp =  pjc/sqrt(pjc**2 + pjn**2)
      angOP= dacos( dotp*0.9999999d0 )*(180.d0/PI)

!In Plane (cs)
      pjc = v_c(l,m).DOT.v_n(l,m+1)
      pjs = v_s(l,m).DOT.v_n(l,m+1)
      dotp = pjs/sqrt( pjc**2 + pjs**2 )
      angIP= dacos( dotp*0.9999999d0 )*(180.d0/PI)
      write(19,'(2i3, 3f10.5)')  l,m,angT, angOP, angIP
!esferico
!phi: nn'
      dotp = v_n(l,m).DOT.v_n(l,m+1)
      phi= dacos( dotp*0.9999999d0 )*(180.d0/PI)
!theta: in plane (cs) mas com c
      pjc = v_c(l,m).DOT.v_n(l,m+1) !n'c
      pjs = v_s(l,m).DOT.v_n(l,m+1) !n's
      dotp = pjc/sqrt( pjc**2 + pjs**2 )
      theta= dacos( dotp*0.9999999d0 )*(180.d0/PI)
      write(20,'(2i3, 2f10.5)')  l,m,theta,phi
...
close(19)
close(20)
! essa versao estah parando apenas na analise de angulo para analise da localizacao.
